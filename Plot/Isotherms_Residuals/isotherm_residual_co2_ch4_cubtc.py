...
df2 = pd.read_csv('iast_pred_px.csv')
...
for ax, i in zip(axes.flat, mole_frac):
    ...
    ax.fill_between(press_gcmc, (upt_ch_gcmc - error_ch), (upt_ch_gcmc + error_ch),color='gray', alpha=0.2)
    ...
    ax.fill_between(press_gcmc, (upt_co_gcmc - error_co), (upt_co_gcmc + error_co),color='gray', alpha=0.2,label="95% Confidence")
    
    ax.grid(True, linestyle='--', alpha=0.4)
    ax.set_title("Adsorption Isotherms Comparison at $X_{CO_{2}}$ = " + f"{i}", fontsize = 15)
    ax.set_xlabel("Pressure (bar)", fontsize = 15)
    ax.set_ylabel("Adsorption Uptake (mg/g)", fontsize = 15)
...
for i, ax in enumerate(axes.flat):
    ax.text(-0.10,1.04,panel_labels[i],transform=ax.transAxes,
        ...
        clip_on=False)
handles, labels = axes.flat[0].get_legend_handles_labels()
fig.legend(handles, labels, loc="upper center", ncol=7, fontsize=15, frameon=False, bbox_to_anchor=(0.5, 0.99))
...
for i in mole_fraction:
    ...
    for ax, j in zip(axes.flat, temp):
        ...
        ax.fill_between(press_gcmc_pxt, (upt_ch_gcmc_pxt - error_ch_pxt), (upt_ch_gcmc_pxt + error_ch_pxt),color='gray', alpha=0.2)
        ...
        ax.fill_between(press_gcmc_pxt, (upt_co_gcmc_pxt - error_co_pxt), (upt_co_gcmc_pxt + error_co_pxt),color='gray', alpha=0.2,label="95% Confidence")
        
        ax.grid(True, linestyle='--', alpha=0.4)
        ax.set_title("Adsorption Isotherms Comparison at $X_{CO_{2}}$ = " + f'{i}, ' + f'Temp = {j}', fontsize = 14)
        ax.set_xlabel("Pressure (bar)", fontsize = 15)
        ax.set_ylabel("Adsorption Uptake (mg/g)", fontsize = 15)
        ax.tick_params(axis='x', labelsize = 12)
        ax.tick_params(axis='y', labelsize = 12)
...
ax[0].set_xlabel("$CH_{4}$ mole fraction", fontsize = 13)
ax[0].set_ylabel("Pressure (bar)", fontsize = 13)
ax[0].set_title("$CH_{4}$ TAL Prediction Residuals for P-X Space", fontsize = 13)
ax[0].tick_params(axis="both", labelsize=12)

contour_co = ax[1].tricontourf(df['CO2_mole_frac'], df['P_BAR'], residual_co, levels=30, cmap="YlGnBu", vmin = vmin, vmax = vmax)
ax[1].set_xlabel("$CO_{2}$ mole fraction", fontsize = 13)
ax[1].set_ylabel("Pressure (bar)", fontsize = 13)
ax[1].set_title("$CO_{2}$ TAL Prediction Residuals for P-X Space", fontsize = 13)
...
for i, axes in enumerate(ax.flat):
    axes.text(-0.15,1.06,panel_labels[i],transform=axes.transAxes,
        fontsize=13,
        fontweight="bold",
        va="top",
        ha="left",
...
font_ppt = {'family': 'sans-serif', 'color': 'black', 'size': 13}
# CH4
all_residuals_ch = []
...
for ax, T in zip(axes.flat, temp):
    ...
    ax.set_xlabel("$CH_{4}$ mole fraction", fontdict = font_ppt)
    ax.set_ylabel("Pressure (bar)", fontdict = font_ppt)
    ax.set_title(f"$CH_{4}$ TAL Prediction Residuals at Temp = {T} K", fontdict= font_ppt)
...
for ax, T in zip(axes.flat, temp):
    ...
    ax.set_xlabel("$CO_{2}$ mole fraction", fontdict = font_ppt)
    ax.set_ylabel("Pressure (bar)", fontdict = font_ppt)
    ax.set_title(f"$CO_{2}$ TAL Prediction Residuals at Temp = {T} K", fontdict= font_ppt)
    ax.tick_params(axis="both", labelsize=12)
