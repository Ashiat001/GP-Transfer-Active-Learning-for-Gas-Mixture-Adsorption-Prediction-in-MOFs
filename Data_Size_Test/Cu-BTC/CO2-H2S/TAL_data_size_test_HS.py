...
#======= Commence training for H2S in P-X space with different training data size =======
...
for i in data_size_px:
    ...
    for j in range(max_iter):
        ...
        err_coll.append(np.mean(rel_error_H2S_M))
    ...
    no_iters = len(err_coll_H2S[i])
    print(no_iters)

    os.system("echo "+str(actual_len)+", "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_pure_hs_hs_px.csv")
...
for i in data_size_pxt:
    ...
    for j in range(max_iter):
        ...
        err_coll_pxt.append(np.mean(rel_error_H2S_pxt))
    ...
    err_coll_H2S_pxt[i] = err_coll_pxt
    ...
    no_iters_pxt = len(err_coll_H2S_pxt[i])
    print(no_iters_pxt)

    os.system("echo "+str(actual_len_pxt)+", "+str(r_squared_pxt)+", "+str(MAE_pxt)+", "+str(MRE_pxt)+", "+str(no_iters_pxt)+"  >> Metrics_pure_hs_hs_pxt.csv")