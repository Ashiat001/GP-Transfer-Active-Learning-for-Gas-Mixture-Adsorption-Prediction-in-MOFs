...
gf = pd.read_csv('training_pure_co_m1.csv')
...
x2 = hf.loc[:, 'CO2_mole_frac']
y2 = hf.loc[:, 'CO2_uptake']
...
c.to_csv('Pred_pure_CO_CO_px_m1.csv',index=False)
...
f.to_csv('TAL_CO_sampled points_px_m1.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_CO2_px, columns = ['GP MRE'])
g.to_csv('TAL_CO_MRE_px_m1.csv', index = False)
...
os.system("echo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_pure_co_co_px_m1.csv")
...
x2_pxt = kf.loc[:, 'CO2_mole_frac']
y2_pxt = kf.loc[:, 'CO2_uptake']
...
j.to_csv('Pred_pure_CO_CO_pxt_m1.csv',index=False)
...
m.to_csv('TAL_CO_sampled points_pxt_m1.csv', index = False)

# write the error collected to a file
n = pd.DataFrame(err_coll_CO2_pxt, columns = ['GP MRE'])
n.to_csv('TAL_CO_MRE_pxt_m1.csv', index = False)
...
os.system("echo "+str(r_squared_pxt)+", "+str(MAE_pxt)+", "+str(MRE_pxt)+", "+str(no_iters_pxt)+"  >> Metrics_pure_co_co_pxt_m1.csv")