...
hf = pd.read_csv('training_mix_pxt_xe.csv')
print(hf.head())
...
for i in range(max_iter):
    ...
    err_coll_Xe_M.append(np.mean(rel_error_Xe_M))
    
    max_Xe_M = np.max(rel_error_Xe_M)
    index_Xe_M = np.argmax(rel_error_Xe_M)

    max_coll_Xe_M.append(max_Xe_M)
    ...
    if PAC_Xe_pxt >= lim:
        ...
        y_data = df2[(round(df2['Pressure(Pa)']) == data_p) & (df2['Temperature(K)'] == data_t) & (round(df2['Xe_mole_frac'], 2) == data_x)]['Xe_uptake']
...
GP_pred, GP_var = GPR.gp_predict(Xe_model_m, x_test_pred)
...
c.to_csv('Pred_AL_Xe_pxt.csv',index=False)
...
f.to_csv('AL_Xe_sampled_points_pxt.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_Xe_M, columns = ['GP MRE'])
g.to_csv('AL_Xe_MRE_pxt.csv', index = False)

# write the PAC collected to a file
p = pd.DataFrame(pac_coll_Xe_pxt, columns = ['PAC'])
p.to_csv('AL_Xe_PAC_pxt.csv', index = False)
...
no_iters = len(err_coll_Xe_M)
print(no_iters)

os.system("echo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_AL_Xe_pxt.csv")