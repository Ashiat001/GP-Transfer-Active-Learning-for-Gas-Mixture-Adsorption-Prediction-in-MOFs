...
gf = pd.read_csv('training_pure_ch_m1.csv')
...
hf = pd.read_csv('training_px_Krish.csv')
print(hf.head())

# Define training and test data and standardize

p = hf.loc[:, 'Pressure(Pa)']
x2 = hf.loc[:, 'CH4_mole_frac']
y2 = hf.loc[:, 'CH4_uptake']
...
c.to_csv('Pred_pure_CH_CH_px_m1.csv',index=False)
...
f.to_csv('TAL_CH_sampled points_px_m1.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_CH4_px, columns = ['GP MRE'])
g.to_csv('TAL_CH_MRE_px_m1.csv', index = False)
...
os.system("echo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_pure_ch_ch_px_m1.csv")

#======= Commence Training for CH4 in P-X-T Space ========
# Read in the mixture training and complete data
kf = pd.read_csv('training_pxt_Krish.csv')
print(kf.head())

# Define training and test data and standardize

p_pxt = kf.loc[:, 'Pressure(Pa)']
t = kf.loc[:, 'Temperature(K)']
x2_pxt = kf.loc[:, 'CH4_mole_frac']
y2_pxt = kf.loc[:, 'CH4_uptake']
...
j.to_csv('Pred_pure_CH_CH_pxt_m1.csv',index=False)
...
m.to_csv('TAL_CH_sampled points_pxt_m1.csv', index = False)

# write the error collected to a file
n = pd.DataFrame(err_coll_CH4_pxt, columns = ['GP MRE'])
n.to_csv('TAL_CH_MRE_pxt_m1.csv', index = False)
...
os.system("echo "+str(r_squared_pxt)+", "+str(MAE_pxt)+", "+str(MRE_pxt)+", "+str(no_iters_pxt)+"  >> Metrics_pure_ch_ch_pxt_m1.csv")
