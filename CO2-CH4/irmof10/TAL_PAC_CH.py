...
df = pd.read_csv('Loadings_methane_IRMOF-10-2.csv')
...
data_mix = GPR.train_data_generator(df2, 10, 40, 0, 3, 5, 1)
...
x2_test = df2.loc[:, 'Mole fraction CH4']
x2_test = x2_test.dropna()
x2_g = df2.loc[:, 'Mole fraction CH4']
x2_g = x2_g.dropna()
...
for i in range(max_iter):
    ...
    if PAC_CH4_px >= lim:
        ...
        y_data = df2[(round(df2['Pressure [Pa]']) == data_p) & (round(df2['Mole fraction CH4'], 2) == data_x)]['Uptake_CH4 [mg/gr]']
        y_data = np.atleast_2d(y_data).flatten().reshape(-1,1)
