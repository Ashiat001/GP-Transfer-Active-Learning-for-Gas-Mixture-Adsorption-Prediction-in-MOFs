...
hf = pd.read_csv('training_mix_pxt_co.csv')
print(hf.head())
...
for i in range(max_iter):
    ...
    err_coll_CO2_M.append(np.mean(rel_error_CO2_M))
    
    max_CO2_M = np.max(rel_error_CO2_M)
    index_CO2_M = np.argmax(rel_error_CO2_M)

    max_coll_CO2_M.append(max_CO2_M)
    ...
    if PAC_CO2_pxt >= lim:
        ...
        y_data = df2[(round(df2['Pressure(Pa)']) == data_p) & (df2['Temperature(K)'] == data_t) & (round(df2['CO2_mole_frac'], 2) == data_x)]['CO2_uptake']
...
GP_pred, GP_var = GPR.gp_predict(co2_model_m, x_test_pred)
...
c.to_csv('Pred_AL_CO_pxt.csv',index=False)
...
f.to_csv('AL_co_sampled points_pxt.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_CO2_M, columns = ['GP MRE'])
g.to_csv('AL_CO_MRE_pxt.csv', index = False)

# write the PAC collected to a file
p = pd.DataFrame(pac_coll_CO2_pxt, columns = ['PAC'])
p.to_csv('AL_CO_PAC_pxt.csv', index = False)
...
no_iters = len(err_coll_CO2_M)
print(no_iters)

os.system("echo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_AL_co_pxt.csv")