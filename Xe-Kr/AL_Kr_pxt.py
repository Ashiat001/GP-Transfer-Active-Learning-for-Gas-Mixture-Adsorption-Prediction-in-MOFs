...
df2 = pd.read_csv('complete_Krish_pxt.csv')
print(df2.head())

hf = pd.read_csv('training_mix_pxt_kr.csv')
print(hf.head())
...
for i in range(max_iter):
    ...
    err_coll_Kr_M.append(np.mean(rel_error_Kr_M))
    
    max_Kr_M = np.max(rel_error_Kr_M)
    index_Kr_M = np.argmax(rel_error_Kr_M)

    max_coll_Kr_M.append(max_Kr_M)
    ...
    if PAC_Kr_pxt >= lim:
        ...
        y_data = df2[(round(df2['Pressure(Pa)']) == data_p) & (df2['Temperature(K)'] == data_t) & (round(df2['Kr_mole_frac'], 2) == data_x)]['Kr_uptake']
...
GP_pred, GP_var = GPR.gp_predict(Kr_model_m, x_test_pred)
...
c.to_csv('Pred_AL_Kr_pxt.csv',index=False)
...
f.to_csv('AL_kr_sampled points_pxt.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_Kr_M, columns = ['GP MRE'])
g.to_csv('AL_Kr_MRE_pxt.csv', index = False)

# write the PAC collected to a file
p = pd.DataFrame(pac_coll_Kr_pxt, columns = ['PAC'])
p.to_csv('AL_Kr_PAC_pxt.csv', index = False)
...
no_iters = len(err_coll_Kr_M)
print(no_iters)

os.system("echo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_AL_kr_pxt.csv")