...
data_mix_px = GPR.train_data_generator(df2, 40, 40, 0, 4, 6, 2, 1)
print(data_mix_px.shape)
data_mix_px.to_csv('training_mix_px_xe.csv', index = False)

data_mix_pxt = GPR.train_data_generator(df3, 20, 40, 0, 4, 6, 2, 1)
print(data_mix_pxt.shape)
data_mix_pxt.to_csv('training_mix_pxt_xe.csv', index = False)
...
#======= Commence training for Xe in P-X space =======
# Read in the mixture training data
hf = pd.read_csv('training_mix_px_xe.csv')
print(hf.head())
...
for i in range(max_iter):
    ...
    for i in range(len(sigma_Xe_M)):
        ...
        if rel_error_Xe_M[i] < 0.02:
            conf_Xe_px += 1
        else:
            nconf_Xe_px += 1
    print(rel_error_Xe_M)

    err_coll_Xe_px.append(np.mean(rel_error_Xe_M))
    
    max_Xe_M = np.max(rel_error_Xe_M)
    index_Xe_M = np.argmax(rel_error_Xe_M)

    max_coll_Xe_px.append(max_Xe_M)

    PAC_Xe_px = 100*(conf_Xe_px/(conf_Xe_px + nconf_Xe_px))
    # print(PAC_Xe)
    pac_coll_Xe_px.append(PAC_Xe_px)
...
c.to_csv('Pred_pure_Xe_Xe_px.csv',index=False)
...
f.to_csv('TAL_Xe_sampled points_px.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_Xe_px, columns = ['GP MRE'])
g.to_csv('TAL_Xe_MRE_px.csv', index = False)

# write the PAC collected to a file
r = pd.DataFrame(pac_coll_Xe_px, columns = ['PAC'])
r.to_csv('TAL_Xe_PAC_px.csv', index = False)
...
no_iters = len(err_coll_Xe_px)
print(no_iters)

os.system("echo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_pure_Xe_Xe_px.csv")

#======= Commence Training for Xe in P-X-T Space ========
# Read in the mixture training and complete data
kf = pd.read_csv('training_mix_pxt_xe.csv')
print(kf.head())
...
for i in range(max_iter):
    ...
    for i in range(len(sigma_Xe_pxt)):
        ...
        if rel_error_Xe_pxt[i] < 0.02:
            conf_Xe_pxt += 1
        else:
            nconf_Xe_pxt += 1
    print(rel_error_Xe_pxt)

    err_coll_Xe_pxt.append(np.mean(rel_error_Xe_pxt))
    
    max_Xe_pxt = np.max(rel_error_Xe_pxt)
    index_Xe_pxt = np.argmax(rel_error_Xe_pxt)

    max_coll_Xe_pxt.append(max_Xe_pxt)

    PAC_Xe_pxt = 100*(conf_Xe_pxt/(conf_Xe_pxt + nconf_Xe_pxt))
    # print(PAC_Xe)
    pac_coll_Xe_pxt.append(PAC_Xe_pxt)
...
j.to_csv('Pred_pure_Xe_Xe_pxt.csv',index=False)
...
m.to_csv('TAL_Xe_sampled points_pxt.csv', index = False)

# write the error collected to a file
n = pd.DataFrame(err_coll_Xe_pxt, columns = ['GP MRE'])
n.to_csv('TAL_Xe_MRE_pxt.csv', index = False)

# write the PAC collected to a file
p = pd.DataFrame(pac_coll_Xe_pxt, columns = ['PAC'])
p.to_csv('TAL_Xe_PAC_pxt.csv', index = False)
...
no_iters_pxt = len(err_coll_Xe_pxt)
print(no_iters_pxt)

os.system("echo "+str(r_squared_pxt)+", "+str(MAE_pxt)+", "+str(MRE_pxt)+", "+str(no_iters_pxt)+"  >> Metrics_pure_Xe_Xe_pxt.csv")