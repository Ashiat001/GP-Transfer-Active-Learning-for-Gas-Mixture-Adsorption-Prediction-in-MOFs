...
for i, ds in enumerate(data_size_px_ch):
    ax[0].scatter(ds, mre_ch4_px[i]*100, s=no_iters_ch4_px[i] * scale, 
               color='darkgreen', alpha=0.5, zorder=5)

# Connect bubbles with a thin line to show trend
ax[0].plot(data_size_px_co, mre_co2_px*100, color='steelblue', linestyle='--', linewidth=0.8,label = '$CO_{2}$')
ax[0].plot(data_size_px_ch, mre_ch4_px*100, color='darkgreen', linestyle='--', linewidth=0.8, label='$CH_{4}$')
...
ax[0].set_xlabel('Initial Training Data Size', fontsize = 15)
ax[0].tick_params(axis = 'x', labelsize = 12)
ax[0].tick_params(axis = 'y', labelsize = 12)
ax[0].set_ylabel('MRE (%)', fontsize = 15)
...
# Connect bubbles with a thin line to show trend
ax[1].plot(data_size_pxt_co, mre_co2_pxt*100, color='steelblue',  linestyle='--', linewidth=0.8, label='$CO_{2}$')
ax[1].plot(data_size_pxt_ch, mre_ch4_pxt*100, color='darkgreen', linestyle='--', linewidth=0.8, label='$CH_{4}$')
...
ax[1].set_xlabel('Initial Training Data Size', fontsize = 15)
ax[1].set_ylabel('MRE (%)', fontsize = 15)
ax[1].tick_params(axis = 'x', labelsize = 12)
ax[1].tick_params(axis = 'y', labelsize = 12)
ax[1].set_title('TAL Performance vs Starting Mixture Data Size - P-X-T Space', fontsize = 15)
ax[1].legend(loc='best', fontsize = 12)
ax[1].grid(True, linestyle='--', alpha=0.4)
ax[1].text(-0.10,1.02,panel_labels[1],transform=ax[1].transAxes,
        fontsize=14,
        fontweight="bold",
        va="top",
        ha="left",
