...
df = pd.read_csv('Loadings_Kr_Cu-BTC-2.csv')
...
data_pure.to_csv('training_pure_kr.csv', index= False)

data_mix_px = GPR.train_data_generator(df2, 40, 40, 0, 5, 7, 3)
print(data_mix_px.shape)
data_mix_px.to_csv('training_mix_px_kr.csv', index = False)

data_mix_pxt = GPR.train_data_generator(df3, 20, 40, 0, 5, 7, 3, 1)
print(data_mix_pxt.shape)
data_mix_pxt.to_csv('training_mix_pxt_kr.csv', index = False)

#======= Commence Pure Data Model development =======

# Read in the pure Kr training data
gf = pd.read_csv('training_pure_kr.csv')
...
for i in range(max_iter):
    ...
    y_pred_Kr, sigma_Kr = GPR.gp_predict(model, x_test)

    rel_error_Kr = np.zeros(len(sigma_Kr))

    for i in range(len(sigma_Kr)):
        rel_error_Kr[i] = abs(sigma_Kr[i])/abs(y_pred_Kr[i])
    print(rel_error_Kr)
    
    max_Kr = np.max(rel_error_Kr)
    index_Kr = np.argmax(rel_error_Kr)
    
    if max_Kr < lim:
        data = x_test[index_Kr]
        print('Done')
        print(f'Final maximum error for Kr is {rel_error_Kr[index_Kr]}')
        Kr_model = model
        break
    else:
        data = x_test[index_Kr]
        ...
        x_test = np.delete(x_test, index_Kr, axis= 0)
...
#======= Commence training for Kr in P-X space =======
# Read in the mixture training data
hf = pd.read_csv('training_mix_px_kr.csv')
print(hf.head())
...
x2_test = df2.loc[:, 'Kr_mole_frac']
x2_test = x2_test.dropna()
x2_g = df2.loc[:, 'Kr_mole_frac']
x2_g = x2_g.dropna()
...
x_test_Kr = np.zeros((len(p_test), dim))
...
for i in range(len(p_test)):
    for k in range(2):
        if k == 0:
            x_test_Kr[i, k] = p_test[i]
        else:
            x_test_Kr[i, k] = x2_test[i]

# Define mean function from source model
def mean_gen(x, model = Kr_model):
...
for i in range(max_iter):
    ...
    y_pred_Kr_M, sigma_Kr_M = GPR.gp_predict(model, x_test_Kr)

    rel_error_Kr_M = np.zeros(len(sigma_Kr_M))

    for i in range(len(sigma_Kr_M)):
        rel_error_Kr_M[i] = abs(sigma_Kr_M[i])/abs(y_pred_Kr_M[i])

        if rel_error_Kr_M[i] < 0.02:
            conf_Kr_px += 1
        else:
            nconf_Kr_px += 1
    print(rel_error_Kr_M)

    err_coll_Kr_px.append(np.mean(rel_error_Kr_M))
    
    max_Kr_M = np.max(rel_error_Kr_M)
    index_Kr_M = np.argmax(rel_error_Kr_M)

    max_coll_Kr_px.append(max_Kr_M)

    PAC_Kr_px = 100*(conf_Kr_px/(conf_Kr_px + nconf_Kr_px))
    # print(PAC_Kr)
    pac_coll_Kr_px.append(PAC_Kr_px)
    
    if PAC_Kr_px >= lim:
        print('Done')
        print(f'Final maximum error for Kr in mixture is {rel_error_Kr_M[index_Kr_M]}')
        Kr_model_px = model
        break
    else:
        data = x_test_Kr[index_Kr_M]

        x_s2 = np.append(x_s2, data)
        x_s2 = x_s2.reshape(-1,dim)

        x_test_Kr = np.delete(x_test_Kr, index_Kr_M, axis= 0)
        x_test_Kr = np.atleast_2d(x_test_Kr).flatten().reshape(-1, dim)
        ...
        y_data = df2[(round(df2['Pressure(Pa)']) == data_p) & (round(df2['Kr_mole_frac'], 2) == data_x)]['Kr_uptake']
        y_data = np.atleast_2d(y_data).flatten().reshape(-1,1)
...
GP_pred, GP_var = GPR.gp_predict(Kr_model_px, x_test_pred)
...
c.to_csv('Pred_pure_Kr_Kr_px.csv',index=False)
...
f.to_csv('TAL_Kr_sampled points_px.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_Kr_px, columns = ['GP MRE'])
g.to_csv('TAL_Kr_MRE_px.csv', index = False)

# write the PAC collected to a file
r = pd.DataFrame(pac_coll_Kr_px, columns = ['PAC'])
r.to_csv('TAL_Kr_PAC_px.csv', index = False)

# Calculate metrics for the model and write to file
y_g = df2.loc[:, 'Kr_uptake']
...
no_iters = len(err_coll_Kr_px)
print(no_iters)

os.system("echo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_pure_kr_kr_px.csv")

#======= Commence Training for Kr in P-X-T Space ========
# Read in the mixture training and complete data
kf = pd.read_csv('training_mix_pxt_kr.csv')
print(kf.head())
...
x2_test_pxt = df3.loc[:, 'Kr_mole_frac']
x2_test_pxt = x2_test_pxt.dropna()
x2_g_pxt = df3.loc[:, 'Kr_mole_frac']
...
x_test_Kr_pxt = np.zeros((len(p_test_pxt), dim))
...
for i in range(len(p_test_pxt)):
    for k in range(dim):
        if k == 0:
            x_test_Kr_pxt[i, k] = p_test_pxt[i]
        elif k == 1:
            x_test_Kr_pxt[i, k] = t_test[i]
        else:
            x_test_Kr_pxt[i, k] = x2_test_pxt[i]
...
for i in range(max_iter):
    ...
    y_pred_Kr_pxt, sigma_Kr_pxt = GPR.gp_predict(model, x_test_Kr_pxt)

    rel_error_Kr_pxt = np.zeros(len(sigma_Kr_pxt))

    for i in range(len(sigma_Kr_pxt)):
        rel_error_Kr_pxt[i] = abs(sigma_Kr_pxt[i])/abs(y_pred_Kr_pxt[i])

        if rel_error_Kr_pxt[i] < 0.02:
            conf_Kr_pxt += 1
        else:
            nconf_Kr_pxt += 1
    print(rel_error_Kr_pxt)

    err_coll_Kr_pxt.append(np.mean(rel_error_Kr_pxt))
    
    max_Kr_pxt = np.max(rel_error_Kr_pxt)
    index_Kr_pxt = np.argmax(rel_error_Kr_pxt)

    max_coll_Kr_pxt.append(max_Kr_pxt)

    PAC_Kr_pxt = 100*(conf_Kr_pxt/(conf_Kr_pxt + nconf_Kr_pxt))
    # print(PAC_Kr)
    pac_coll_Kr_pxt.append(PAC_Kr_pxt)
    
    if PAC_Kr_pxt >= lim:
        print('Done')
        print(f'Final maximum error for Kr in mixture is {rel_error_Kr_pxt[index_Kr_pxt]}')
        Kr_model_pxt = model
        break
    else:
        data = x_test_Kr_pxt[index_Kr_pxt]

        x_s2_pxt = np.append(x_s2_pxt, data)
        x_s2_pxt = x_s2_pxt.reshape(-1,dim)

        x_test_Kr_pxt = np.delete(x_test_Kr_pxt, index_Kr_pxt, axis= 0)
        x_test_Kr_pxt = np.atleast_2d(x_test_Kr_pxt).flatten().reshape(-1, dim)
        ...
        data_x = round(data_x, 2)
        y_data = df3[(round(df3['Pressure(Pa)']) == data_p) & (df3['Temperature(K)'] == data_t) & (round(df3['Kr_mole_frac'], 2) == data_x)]['Kr_uptake']
        y_data = np.atleast_2d(y_data).flatten().reshape(-1,1)
...
# make predictions on the data using the trained model
GP_pred_pxt, GP_var_pxt = GPR.gp_predict(Kr_model_pxt, x_test_pred_pxt)
...
j.to_csv('Pred_pure_Kr_Kr_pxt.csv',index=False)
...
m.to_csv('TAL_Kr_sampled points_pxt.csv', index = False)

# write the error collected to a file
n = pd.DataFrame(err_coll_Kr_pxt, columns = ['GP MRE'])
n.to_csv('TAL_Kr_MRE_pxt.csv', index = False)

# write the PAC collected to a file
p = pd.DataFrame(pac_coll_Kr_pxt, columns = ['PAC'])
p.to_csv('TAL_Kr_PAC_pxt.csv', index = False)

# Calculate metrics for the model and write to file
y_g_pxt = df3.loc[:, 'Kr_uptake']
...
no_iters_pxt = len(err_coll_Kr_pxt)
print(no_iters_pxt)

os.system("echo "+str(r_squared_pxt)+", "+str(MAE_pxt)+", "+str(MRE_pxt)+", "+str(no_iters_pxt)+"  >> Metrics_pure_kr_kr_pxt.csv")