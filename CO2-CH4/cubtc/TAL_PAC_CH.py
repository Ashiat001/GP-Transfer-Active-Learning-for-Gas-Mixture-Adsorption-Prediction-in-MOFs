...
data_mix = GPR.train_data_generator(df2, 20, 40, 0, 5, 7, 3)
print(data_mix.shape)
data_mix.to_csv('training_mix_px_ch.csv', index = False)

data_mix_pxt = GPR.train_data_generator(df3, 5, 40, 0, 5, 7, 3, 1)
print(data_mix_pxt.shape)
data_mix_pxt.to_csv('training_mix_pxt_ch.csv', index = False)
...
for i in range(max_iter):
    ...
    PAC_CH4 = 100*(conf_CH4/(conf_CH4 + nconf_CH4))
    # print(PAC_CH4)
...
#======= Commence training for CH4 in P-X space =======
# Read in the mixture training data
hf = pd.read_csv('training_mix_px_ch.csv')
print(hf.head())
...
for i in range(max_iter):
    ...
    err_coll_CH4_px.append(np.mean(rel_error_CH4_M))
    
    max_CH4_M = np.max(rel_error_CH4_M)
    index_CH4_M = np.argmax(rel_error_CH4_M)

    max_coll_CH4_px.append(max_CH4_M)

    PAC_CH4_px = 100*(conf_CH4_px/(conf_CH4_px + nconf_CH4_px))
    # print(PAC_CH4)
    pac_coll_CH4_px.append(PAC_CH4_px)
...
c.to_csv('Pred_pure_CH_CH_px.csv',index=False)
...
f.to_csv('TAL_CH_sampled points_px.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_CH4_px, columns = ['GP MRE'])
g.to_csv('TAL_CH_MRE_px.csv', index = False)

# write the PAC collected to a file
r = pd.DataFrame(pac_coll_CH4_px, columns = ['PAC'])
r.to_csv('TAL_CH_PAC_px.csv', index = False)
...
no_iters = len(err_coll_CH4_px)
print(no_iters)

os.system("echo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_pure_ch_ch_px.csv")

#======= Commence Training for CH4 in P-X-T Space ========
# Read in the mixture training and complete data
kf = pd.read_csv('training_mix_pxt_ch.csv')
print(kf.head())
...
for i in range(max_iter):
    ...
    for i in range(len(sigma_CH4_pxt)):
        ...
        if rel_error_CH4_pxt[i] < 0.02:
            conf_CH4_pxt += 1
        else:
            nconf_CH4_pxt += 1
    print(rel_error_CH4_pxt)

    err_coll_CH4_pxt.append(np.mean(rel_error_CH4_pxt))
    
    max_CH4_pxt = np.max(rel_error_CH4_pxt)
    index_CH4_pxt = np.argmax(rel_error_CH4_pxt)

    max_coll_CH4_pxt.append(max_CH4_pxt)
...
j.to_csv('Pred_pure_CH_CH_pxt.csv',index=False)
...
m.to_csv('TAL_CH_sampled points_pxt.csv', index = False)

# write the error collected to a file
n = pd.DataFrame(err_coll_CH4_pxt, columns = ['GP MRE'])
n.to_csv('TAL_CH_MRE_pxt.csv', index = False)

# write the PAC collected to a file
p = pd.DataFrame(pac_coll_CH4_pxt, columns = ['PAC'])
p.to_csv('TAL_CH_PAC_pxt.csv', index = False)
...
no_iters_pxt = len(err_coll_CH4_pxt)
print(no_iters_pxt)

os.system("echo "+str(r_squared_pxt)+", "+str(MAE_pxt)+", "+str(MRE_pxt)+", "+str(no_iters_pxt)+"  >> Metrics_pure_ch_ch_pxt.csv")
