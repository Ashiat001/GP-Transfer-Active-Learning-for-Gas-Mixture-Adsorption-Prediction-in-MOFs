...
hf = pd.read_csv('training_mix_pxt_hs.csv')
print(hf.head())
...
for i in range(max_iter):
    ...
    err_coll_H2S_M.append(np.mean(rel_error_H2S_M))
    
    max_H2S_M = np.max(rel_error_H2S_M)
    index_H2S_M = np.argmax(rel_error_H2S_M)

    max_coll_H2S_M.append(max_H2S_M)
    ...
    if PAC_H2S_pxt >= lim:
        ...
        y_data = df2[(round(df2['Pressure(Pa)']) == data_p) & (df2['Temperature(K)'] == data_t) & (round(df2['H2S_mole_frac'], 2) == data_x)]['H2S_uptake']
...
GP_pred, GP_var = GPR.gp_predict(h2s_model_m, x_test_pred)
...
c.to_csv('Pred_AL_HS_pxt.csv',index=False)
...
f.to_csv('AL_HS_sampled_points_pxt.csv', index = False)

# write the error collected to a file
g = pd.DataFrame(err_coll_H2S_M, columns = ['GP MRE'])
g.to_csv('AL_HS_MRE_pxt.csv', index = False)

# write the PAC collected to a file
p = pd.DataFrame(pac_coll_H2S_pxt, columns = ['PAC'])
p.to_csv('AL_HS_PAC_pxt.csv', index = False)
...
no_iters = len(err_coll_H2S_M)
print(no_iters)

os.system("eHSo "+str(r_squared)+", "+str(MAE)+", "+str(MRE)+", "+str(no_iters)+"  >> Metrics_AL_hs_pxt.csv")